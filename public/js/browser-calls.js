...
/* Call a customer from a contact */
...
function initNewContactForm() {
  ...
  formEl.find("[type='button']").click(function(e) {
    ...
    .done(function(){
      showNotification("Contact was created successfully.", "success")
      // clear form
      formEl.find("input[type=text], textarea").val("");
    })
    .fail(function(res) {
      showNotification("Contact creation failed. " + res.responseText, "danger")
    });
  });
}
...
function showNotification(text, style) {
  ...
  var alertEl = $(".alert.contact-support-notifications");

  if (alertEl.length == 0) {
    alertEl = $("<div class=\"alert contact-support-notifications\"></div>");
    $("body").before(alertEl);
  }
